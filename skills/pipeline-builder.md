...
OVERSAMPLE (critical): verified owner-level decision-makers are sparse, so the BUSINESS fetch must be large (number_of_results 200-400 — it is free exploration). Verify events, drop AI/tech/venture and over-ceiling companies, then enrich + retrieve only the survivors up to the cap. Email is no longer a hard gate (no-email leads ship as Group B with LinkedIn-only outreach), which widens deliverable yield, but a large pool is still needed for signal + profile quality. A small business fetch yields almost no verified leads.
...
- FILL IF SHORT (headroom fallback, LIVE fills the lead target): quality-first, the tight ICP filter is the primary pass. ONLY if it yields fewer than the target (Tier 1 + Tier 2, after dedup) do you broaden and repeat the fetch+verify+enrich to backfill the gap, stopping the moment you reach the target: Round 1 broaden SIZE (add the next-larger company_size bands, e.g. 501-1000, 1001-5000, from context `broaden_size`); Round 2 broaden SECTOR (add adjacent linkedin_category from context `broaden_categories`, or sectors the client credibly serves). Every broadened lead still needs provenance (source link or signalProof) and a personal linkedin.com/in/ profile, still excludes AI/tech/venture, ranks Tier 1 above Tier 2 and email-leads above no-email leads, dedups against earlier rounds, and stays within the credit cap.
- If even the broadened pool is exhausted, deliver what qualifies and report the count. Never pad with unverified leads to hit a number.
...
fetch-entities(entity_type:"prospects", businesses_reference_table:[from 2a],
  ...
  filters:{ job_title, job_level })   # do NOT pre-filter on has_email; email is decided after enrich
```
VERIFY (mandatory, before enrich): call `fetch-businesses-events` on the businesses_reference_table from 2a, event_types = your event list, timestamp_from = window days ago. Read each record's `data.link`, `data.title`, `data.description`, and type-specific fields (roles/department for hiring, partner_company for M&A, court/case for legal). Classify each company into a TRUST TIER: Tier 1 = the event has a real public `data.link` (http...), keep the link for a clickable source; Tier 2 = a genuine detected event with no public link but real structured detail, build a RICH `signalProof` from every type-specific field returned (e.g. the specific roles posted, the M&A partner, the court/case), stated as a detected business signal. Drop a company only if it has NEITHER a link NOR any structured detail. Tier 1 ranks above Tier 2; Tier 2 backfills only when Tier 1 is short. Then refine 2b to the surviving companies only.
ENRICH once: enrich-prospects-contacts, contact_types ["email"] (both modes). Then UNMASK by mode: DEMO show-sample, LIVE export-to-csv (limit = fetch). A lead needs a verified signal (Tier 1 link OR Tier 2 signalProof) AND a personal linkedin.com/in/ profile. Email is NOT required: order rows so GROUP A (verified signal + email) come first, then GROUP B (verified signal, no email, set email "" and has_email false) backfills toward the target. Within each group, Tier 1 before Tier 2.
